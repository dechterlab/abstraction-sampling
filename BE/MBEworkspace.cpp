...
#endif 
{
    ...
    pthread_mutex_unlock(&BucketElimination::MBEworkspace::stopSignalMutex);
		...
		int32_t res = b->ComputeOutputFunctions(true, approx_bound) ;
		bool do_break = false ;
/*		if (NULL != bews->logFile()) {
...
int32_t BucketElimination::MBEworkspace::Initialize(ARE::ARP & Problem, bool UseLogScale, const int32_t *VarOrderingAsVarList, int32_t DeleteUsedTables)
{
	...
	_lenComputationOrder = 0 ;
	_VarOrdering_InducedWidth = Problem.VarOrdering_InducedWidth() ;
	_PseudoWidth = -1 ;
	for (i = 0 ; i < _nVars ; i++) 
...
int32_t BucketElimination::MBEworkspace::CreateBuckets(bool ANDORtree, bool KeepBTsignature, bool SimplifyBTstructure, bool CreateSuperBuckets)
{
	if (_nVars <= 0) 
		return 0 ;
...
//	if (NULL == _BTchildlistStorage) 
//		goto failed ;
	if (_VarOrdering_InducedWidth > 0) {
		TempSpaceForArglist = new int32_t[_VarOrdering_InducedWidth] ;
		if (NULL == TempSpaceForArglist) 
			goto failed ;
		TempSpaceForArglistSize = _VarOrdering_InducedWidth ;
		}
	...
	{
	std::vector<int32_t> temp_children_storage ; temp_children_storage.reserve(100) ;
...
//		nBTchildlistStorageUsed += nchildren ;
	...
	if (SimplifyBTstructure) {
		...
		for (j = i = 0 ; i < _nBuckets ; ++i) {
			if (NULL != _Buckets[i]) {
				_Buckets[j] = _Buckets[i] ;
				_Buckets[j]->SetIDX(j) ;
				++j ;
				}
			}
...
//	int32_t original_i = _iBound ;
	...
	_iBound = MinIBound ;
	if (0 != CreateMBPartitioning(false, false, 0, 0)) {
	...
	iUpper = _VarOrdering_InducedWidth > 0 ? _VarOrdering_InducedWidth+1 : _nVars ;
	iLowerspace = iMINspace ;
	iLNP = _nBucketsWithPartitioning ;
	iMaxDepthPartitionedBucket = _maxDepthPartitionedBucket ;
//goto done ;
	if (_VarOrdering_InducedWidth > 0 && _VarOrdering_InducedWidth < 2) {
		...
		if (0 != _nBucketsWithPartitioning) {
			if (NULL != ARE::fpLOG) {
				fprintf(ARE::fpLOG, "\n   BucketElimination::MBEworkspace::FindIBoundForSpaceAllowed; ERROR : var_order w*(%d)<2 and i-bound=%d, but there is partitioning ...", (int32_t) _VarOrdering_InducedWidth, (int32_t) _iBound) ;
				::fflush(ARE::fpLOG) ;
				}
			goto failed ;
			}
	...
	if (0 != _nBucketsWithPartitioning) {
		if (NULL != ARE::fpLOG) {
			fprintf(ARE::fpLOG, "\n   BucketElimination::MBEworkspace::FindIBoundForSpaceAllowed; ERROR : ibound(%d) >= var_order w*(%d)+1, but there is partitioning ...", (int32_t) _iBound, (int32_t) _VarOrdering_InducedWidth) ;
			::fflush(ARE::fpLOG) ;
			}
		goto failed ;
		}
...
int32_t BucketElimination::MBEworkspace::CreateMBPartitioning(bool CreateTables, bool doMomentMatching, signed char Bound, int32_t ComputeComputationOrder)
{
...
/*	for (int32_t i = _Problem->N()-1 ; i >= 0 ; --i) {
		...
		if (NULL == b) continue ;*/
		int32_t res = b->CreateMBPartitioning(_iBound, CreateTables, doMomentMatching, Bound, abandon, key, data, helperArray) ;
		if (0 != res) 
			goto failed ;
	...
	if (_VarOrdering_InducedWidth > 0 && _iBound >= _VarOrdering_InducedWidth) {
		if (_nBucketsWithPartitioning > 0) {
			if (NULL != ARE::fpLOG) {
				fprintf(ARE::fpLOG, "\n   BucketElimination::MBEworkspace::CreateMBPartitioning ERROR : _iBound(%d) >= var_order w*(%d) but there is partitioning ...", (int32_t) _iBound, (int32_t) _VarOrdering_InducedWidth) ;
				::fflush(ARE::fpLOG) ;
				}
			int32_t error = 1 ;
			}
...
int32_t BucketElimination::MBEworkspace::ComputeOutputFunctions(bool DoMomentMatching, signed char approx_bound)
{
	int32_t i ;
	...
	for (int32_t i = _lenComputationOrder-1 ; i >= 0 ; --i) {
		...
		if (NULL == b) 
			return 1 ;
		int32_t res_mbe = b->ComputeOutputFunctions(DoMomentMatching, approx_bound) ;
		if (0 != res_mbe) 
			return res_mbe ;
		}
...
int32_t BucketElimination::MBEworkspace::SaveReducedProblem(signed char ReducedProblemType, signed char ApproximationBound, std::string & fn, int32_t & nKeepVariables, std::vector<int32_t> & Old2NewVarMap, std::vector<ARE::Function*> & ReducedProblemFunctions)
{
	...
	if (ReducedProblemType <= 0) {
		...
		for (i = _Problem->N()-1 ; i >= 0 ; i--) {
			...
			// MB partitioning
			bool abandon = true ;
			int32_t res = b->CreateMBPartitioning(iBound, false, false, ApproximationBound, abandon, key, data, helperArray) ;
			...
			// generate output fn
			if (0 != b->ComputeOutputFunctions(false, ApproximationBound)) 
				goto done ;
			}
		}
