...
public :
	...
	inline int32_t SetChildren(int32_t n, int32_t *C)
	{
		...
		if (n > 0 && NULL == _ChildVars) {
			_ChildVars = new int32_t[n] ;
			if (NULL == _ChildVars) 
				return 1 ;
			}
		_nChildren = n ;
		if (n > 0) 
			memcpy(_ChildVars, C, n*sizeof(int32_t)) ;
		return 0 ;
	}
...
//	int32_t ReorderFunctionScopesWrtParentBucket(bool IncludeOriginalFunctions, bool IncludeNewFunctions) ;
	...
	int32_t ComputeOutputFunctions(bool DoMomentMatching, signed char Bound) ;
