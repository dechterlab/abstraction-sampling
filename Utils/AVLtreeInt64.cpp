...
#ifdef DEBUG_AVL_new
CMauiAVL64Tree *_msTestAVLTree = NULL ;
#endif // DEBUG_AVL_new
...
void MauiAVL64Node::operator<<(const MauiAVL64Node & Node)
{
	m_id = Node.m_id ;
...
void CMauiAVL64Tree::Empty(void)
{
	unsigned long i ;
...
void CMauiAVL64Tree::SetReallocationSize(unsigned long reallocation_size)
{
...
int CMauiAVL64Tree::AllocateMemory(unsigned long new_size)
{
	unsigned long i ;
	...
	if (new_size > 0) 
		m_db_tree = new MauiAVL64Node[new_size] ;
	else 
		return 1 ;
	if (NULL == m_db_tree) {
		m_allocated_space = 0 ;
...
int CMauiAVL64Tree::ReAllocateMemory(void)
{
	unsigned long new_size, i ;
	MauiAVL64Node *new_db_tree = NULL ;
	...
	new_db_tree = new MauiAVL64Node[new_size] ;
	if (NULL == new_db_tree) return 0 ;

	// copy the entire old tree into the new tree
	memcpy(new_db_tree,m_db_tree,(size_t) i*sizeof(MauiAVL64Node)) ;
...
CMauiAVL64Tree::CMauiAVL64Tree(void)
{
	m_db_tree = NULL ;
	m_allocated_space = 0 ;
	m_size = 0 ;
...
CMauiAVL64Tree::CMauiAVL64Tree(unsigned long tree_size /* number of keys (>= 0) */, 
 ...
 * ********** ********** */
	...
	if (tree_size > 0) {
		...
		QuickSorti64_i64(key,tree_size,data,left,right) ;
		}
...
void CMauiAVL64Tree::PrintTree(void)
{
	// TODO.
}
...
long CMauiAVL64Tree::CheckTree(char **pErrorStr)
{
	unsigned long i, j, m ;
	unsigned long k ;
...
void CMauiAVL64Tree::R_rotation(unsigned long k)
...
void CMauiAVL64Tree::L_rotation(unsigned long k)
...
void CMauiAVL64Tree::BalanceInsertTree(void)
// balance the tree
...
void CMauiAVL64Tree::BalanceDeleteTree(void)
...
long CMauiAVL64Tree::Find(int64_t id, int64_t *data /* return data here */)  
{
#ifdef DEBUG_AVL_mutex
...
long CMauiAVL64Tree::FindNext(int64_t key, int64_t *next_key, int64_t *next_data /* return data here */)  
{
	long j ;
	if ((j = Find(key, NULL))) {
...
long CMauiAVL64Tree::Increment(int64_t id)
{
	long j ;
...
long CMauiAVL64Tree::Adjust(int64_t id, int64_t data_adjustment, bool OnlyIfExists)
{
	long j ;
...
long CMauiAVL64Tree::Decrement(int64_t id, int remove_if_0)
{
	long j ;
...
int CMauiAVL64Tree::SetData(unsigned long idx, int64_t data)
{
...
long CMauiAVL64Tree::Insert(int64_t id, int64_t data)
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
*/
	...
	if (NULL == _msTestAVLTree) {
		try {
			_msTestAVLTree = new CMauiAVL64Tree ;
			}
		catch (...) {
			}
		}
...
long CMauiAVL64Tree::RemoveFirst(int64_t *key /* output */, int64_t *data /* return data here. can be NULL */) // returns TRUE iff everything is fine
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
long CMauiAVL64Tree::Remove(int64_t id, int64_t *data /* return data here */)
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
//long CMauiAVL64Tree::get_first_key_data(long *key /* can be NULL */, long *data /* can be NULL */)
//{
...
//long CMauiAVL64Tree::get_second_key_data(long *key /* can be NULL */, long *data /* can be NULL */)
//{
...
long CMauiAVL64Tree::GetFirst(int64_t *key /* can be NULL */, int64_t *data /* can be NULL */, long & current) const 
{
	if (m_size < 1) {
		current = 0 ;
		return 0 ;
...
long CMauiAVL64Tree::GetNext(int64_t *key /* can be NULL */, int64_t *data /* can be NULL */, long & current) const 
{
...
long CMauiAVL64Tree::GetNextAndSetData(int64_t *key /* can be NULL */, int64_t *data /* can be NULL */, int64_t NewData, long & current) const 
{
	// if the tree is empty, cannot select anything
	if (m_size < 1) {
		if (key) *key = 0 ;
...
long CMauiAVL64Tree::GetCurrentAndAdvance(int64_t *key /* can be NULL */, int64_t *data /* can be NULL */, long & current) const 
{
	// if the tree is empty, cannot select anything
	if (m_size < 1) {
		if (key) *key = 0 ;
...
long CMauiAVL64Tree::FindSmallestPositiveKeyNotUsed(void)
{
	long i, j, target_key ;
...
int CMauiAVL64Tree::FindData(int64_t data)  
{
	long i ;
...
int CMauiAVL64Tree::DecrementLargerKeys(int64_t key)
{
	long j, start_entity ;
	int64_t i ;
	...
	for (j = start_entity ; j > 0 ; j = m_db_tree[j].m_next_entity) {
		--(m_db_tree[j].m_id) ;
		}
...
long CMauiAVL64Tree::TestTree(int numIterations, char **pErrorStr) // returns TRUE iff the tree is OK.
{
	if (pErrorStr) 
		*pErrorStr = NULL ;
	if (numIterations < 1) 
...
bool CMauiAVL64Tree::operator<<(const CMauiAVL64Tree & AVLtree)
{
	unsigned long i ;
...
bool CMauiAVL64Tree::TestConsistency(long checkbitvector) const
{
