...
#ifndef AVLObj_HXX_INCLUDED
#define AVLObj_HXX_INCLUDED
...
/*
	This structure is a obj node in the AVL tree. Simple node has only id, no data.
*/

typedef struct _MauiAVLNodeObj {

	// obj of this entity (obj has an ID which is the key)
	void *m_id ;
...
public :
	...
	void operator<<(const _MauiAVLNodeObj & Node) ;
...
class CMauiAVLTreeObj
{

protected :

	// compares O1 and O2; return value is TRUE iff O1>O2.
	QuickSortGreaterCompFn _ObjComparator ;

	// pointer to the actual AVL tree
	_MauiAVLNodeObj *m_db_tree ;
...
private :
	...
	bool operator<<(const CMauiAVLTreeObj & AVLtree) ;
...
public :
	...
	// returns >0 iff the key in in the tree
	int32_t Find(void *key, void **KeyInTree) ;
	int32_t FindNext(void *key, void **next_key) ;

	// returns FALSE iff it fails (no memory). if key is in use, it will replace.
	int32_t Insert(void *key) ;
	...
	int32_t Remove(void *key) ; // returns TRUE iff everything is fine
	int32_t RemoveFirst(void **key /* output */) ; // returns TRUE iff everything is fine
...
*/
	...
	int32_t GetFirst(void **key /* can be NULL */, int32_t & current) const ;
	...
	int32_t GetNext(void **key /* can be NULL */, int32_t & current) const ;
	...
	int32_t GetCurrentAndAdvance(void **key /* can be NULL */, int32_t & current) const ;

	void PrintString(char *) { }
	int32_t CheckTree(char **pErrorStr = 0) ; // returns TRUE iff the tree is OK.
...
public :
	...
	CMauiAVLTreeObj(void) ;

	// This constructor creates a tree from a set of keys.
	CMauiAVLTreeObj(QuickSortGreaterCompFn ObjComparator, uint32_t size /* number of keys (has to be >= 0) */, 
