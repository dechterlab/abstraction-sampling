...
#ifdef DEBUG_AVL_new
CMauiAVLTreeObj *_msTestAVLTree = NULL ;
#endif // DEBUG_AVL_new
...
/*
	********** ********** ********** ********** ********** ********** ********** ********** **********
	CMauiAVLTreeObj member functions.
	********** ********** ********** ********** ********** ********** ********** ********** **********
*/

void _MauiAVLNodeObj::operator<<(const _MauiAVLNodeObj & Node)
{
	m_id = Node.m_id ;
...
void CMauiAVLTreeObj::EmptyQuick(void)
{
	if (0 == m_size) 
		return ;
...
void CMauiAVLTreeObj::Empty(void)
{
	uint32_t i ;
...
void CMauiAVLTreeObj::SetReallocationSize(uint32_t reallocation_size)
{
...
int CMauiAVLTreeObj::AllocateMemory(uint32_t new_size)
{
	uint32_t i ;
	...
	if (new_size > 0) 
		m_db_tree = new _MauiAVLNodeObj[new_size] ;
	if (NULL == m_db_tree) {
		m_allocated_space = 0 ;
		m_size = 0 ;
		m_free_size = 0 ;
...
int CMauiAVLTreeObj::ReAllocateMemory(void)
{
	uint32_t new_size, i ;
	_MauiAVLNodeObj *new_db_tree = NULL ;
	...
	new_db_tree = new _MauiAVLNodeObj[new_size] ;
	if (NULL == new_db_tree) return 0 ;

	// copy the entire old tree into the new tree
	memcpy(new_db_tree,m_db_tree,(size_t) i*sizeof(_MauiAVLNodeObj)) ;
...
CMauiAVLTreeObj::CMauiAVLTreeObj(void)
{
	m_db_tree = NULL ;
...
CMauiAVLTreeObj::CMauiAVLTreeObj(QuickSortGreaterCompFn ObjComparator, uint32_t tree_size /* number of keys (>= 0) */, 
	void **key /* an array of keys */, 
	uint32_t space /* how much space to allocate for the AVL tree (> 0) */, 
	uint32_t re_allocation_size)
{
	_ObjComparator = ObjComparator ;
 ...
 * ********** ********** */
	...
	QuickSort(key, tree_size, left, right, _ObjComparator) ;
...
void CMauiAVLTreeObj::PrintTree(void)
{
	// TODO.
}
...
int32_t CMauiAVLTreeObj::CheckTree(char **pErrorStr)
{
	uint32_t i, j, m ;
	uint32_t k ;
...
backtrack_12_07_1994:
		...
		for (m = 0 ; m < k; m++) {
			if (Left[m] > 0) {
				if ((*_ObjComparator)(m_db_tree[Left[k]].m_id, m_db_tree[Left[m]].m_id) > 0) {
...
void CMauiAVLTreeObj::R_rotation(uint32_t k)
...
void CMauiAVLTreeObj::L_rotation(uint32_t k)
...
void CMauiAVLTreeObj::BalanceInsertTree(void)
// balance the tree
...
void CMauiAVLTreeObj::BalanceDeleteTree(void)
...
int32_t CMauiAVLTreeObj::Find(void *id, void **KeyInTree)  
{
	*KeyInTree = NULL ;
#ifdef DEBUG_AVL_mutex
...
down_the_tree_12_04_94:
	...
	int32_t resComp = (*_ObjComparator)(m_db_tree[j].m_id, id) ;
	if (resComp == 0) {
//	if (m_db_tree[j].m_id == id) {
		Left[i] = 0 ;
		path_len = i ;
		*KeyInTree = m_db_tree[j].m_id ;
...
int32_t CMauiAVLTreeObj::FindNext(void *key, void **next_key)  
{
	void *key_in_tree = NULL ;
	int32_t j = Find(key, &key_in_tree) ;
	if (j > 0) {
...
int32_t CMauiAVLTreeObj::Insert(void *id)
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
*/
	...
	if (NULL == _msTestAVLTree) {
		try {
			_msTestAVLTree = new CMauiAVLTreeObj ;
			}
		catch (...) {
			}
		}
...
int32_t CMauiAVLTreeObj::RemoveFirst(void **key /* output */) // returns TRUE iff everything is fine
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
int32_t CMauiAVLTreeObj::Remove(void *id)
{
#ifdef DEBUG_AVL_mutex
	if (NULL == _msAVLdebugMutex) 
		_msAVLdebugMutex = new COptexEXT_debug("AVLtestMutex") ;
...
//int32_t CMauiAVLTreeObj::get_first_key_data(int32_t *key /* can be NULL */, int32_t *data /* can be NULL */)
//{
...
//int32_t CMauiAVLTreeObj::get_second_key_data(int32_t *key /* can be NULL */, int32_t *data /* can be NULL */)
//{
...
int32_t CMauiAVLTreeObj::GetFirst(void **key /* can be NULL */, int32_t & current) const 
{
	if (m_size < 1) {
		current = 0 ;
		return 0 ;
...
int32_t CMauiAVLTreeObj::GetNext(void **key /* can be NULL */, int32_t & current) const 
{
...
int32_t CMauiAVLTreeObj::GetCurrentAndAdvance(void **key /* can be NULL */, int32_t & current) const 
{
	// if the tree is empty, cannot select anything
	if (m_size < 1) {
		if (key) *key = 0 ;
...
bool CMauiAVLTreeObj::operator<<(const CMauiAVLTreeObj & AVLtree)
{
	uint32_t i ;
...
bool CMauiAVLTreeObj::TestConsistency(int32_t checkbitvector) const
{
	...
	for (i = m_first_used_block ; i ; i = m_db_tree[i].m_next_entity) {
		if (m_db_tree[i].m_LC) {
			...
			if ((*_ObjComparator)(m_db_tree[lid].m_id, m_db_tree[i].m_id) > 0) 
//			if (m_db_tree[lid].m_id > m_db_tree[i].m_id) 
				return false ;
			}
		if (m_db_tree[i].m_RC) {
			uint32_t rid = m_db_tree[i].m_RC ;
			if ((*_ObjComparator)(m_db_tree[rid].m_id, m_db_tree[i].m_id) < 0) 
