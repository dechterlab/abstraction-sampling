...
int32_t ARE::Graph::Destroy(void) 
{
	...
	if (NULL != _IgnoreVariables) {
		delete [] _IgnoreVariables ;
		_IgnoreVariables = NULL ; 
		}
...
int32_t ARE::Graph::Create(ARP & Problem) 
{
	...
	::vector<int32_t> & qv = Problem.QueryVariables() ;
	_nIgnoreVariables = qv.size() ;
	if (_nIgnoreVariables > 0) {
		_IgnoreVariables = new int32_t[_nIgnoreVariables] ;
		if (NULL == _IgnoreVariables) 
			{ Destroy() ; return 1 ; }
		}
	for (int ii = 0 ; ii < qv.size() ; ++ii) 
		_IgnoreVariables[ii] = qv[ii] ;
...
int32_t ARE::Graph::operator=(const Graph & G)
{
	...
	if (G._nIgnoreVariables > 0) {
		_IgnoreVariables = new int32_t[G._nIgnoreVariables] ;
		if (NULL == _IgnoreVariables) 
			{ Destroy() ; return 1 ; }
		}
