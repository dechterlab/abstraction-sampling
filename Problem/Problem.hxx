...
public :
	...
	inline bool IsEvidenceVar(int32_t IDX) const { return _Value[IDX] >= 0 ; } 
	...
	int32_t ComputeNumEvidenceVars(void)
	{
		int32_t nEvidenceVars = 0 ;
		for (int32_t i = 0 ; i < _nVars ; ++i) 
			{ if (IsEvidenceVar(i)) nEvidenceVars++ ; }
		return nEvidenceVars ;
	}
...
public :
	...
	// load variable order from a string buffer.
	int32_t LoadVariableOrderingFromBuffer(
		// 0=OrderType means variable elimination order.
		// 1=OrderType means variable bucket-tree order.
		int32_t OrderType,
		// 0 = we assume it is in the format "{var0;var1;...;varN}"
		// 1 = first line is : format : first row = <#vars> <width>; remaining rows = ...; second line is <#vars> <width>; from third line is list of variable indeces. 
		int format, 
		bool test, 
		const char *SerializedVarListInElimOrder) ;
...
protected :
	...
	std::vector<int32_t> _QueryVariables ; // if this is set, we want to compute a distribution on this var
...
public :
	...
	inline std::vector<int32_t> & QueryVariables(void) { return _QueryVariables ; }
	...
	inline bool IsMaximizationProblem(void) const { return VAR_ELIMINATION_TYPE_MAX == _VarElimType ; }
	inline bool IsMinimizationProblem(void) const { return VAR_ELIMINATION_TYPE_MIN == _VarElimType ; }
	inline bool IsSummationProblem(void) const { return VAR_ELIMINATION_TYPE_SUM == _VarElimType ; }
	...
	inline void SetQueryVariables(std::vector<int32_t> & vars)
	{
		_QueryVariables.clear() ;
		for (int32_t ii = 0 ; ii < vars.size() ; ++ii) {
			int32_t v = vars[ii] ;
			if (v >= 0 && v < _nVars) 
				_QueryVariables.push_back(v) ;
			}
	}
...
public :
	...
	int32_t ComputeSingletonConsistency(int32_t & nNewSingletonDomainVariables, int32_t & nVarsWithReducedDomainSize) ;
